...
def _enum_value(value: Any) -> str:
    """Return enum values without assuming every payload field is an enum."""
...
def render_regimen_summary(regimen_summary: dict[str, Any] | None) -> None:
    ...
    st.subheader("Regimen-Level Summary")
    ...
    with st.container(border=True):
        ...
        col_b.metric(
            "Pairs with hits",
            regimen_summary.get("pair_count_with_hits", 0),
        )
        col_c.metric(
            "Pairwise hits",
            regimen_summary.get("hit_counts", {}).get("total", 0),
        )

        overall_severity = _enum_value(regimen_summary["overall_severity"])
        overall_class = _enum_value(regimen_summary["overall_rule_class"])
    ...
    if flags:
        with st.container(border=True):
            st.markdown("### Regimen-Wide Educational Flags")
            for flag in flags:
                st.warning(flag.get("message", ""))

    pd_stacks = regimen_summary.get("pd_stacks", [])
    if pd_stacks:
        with st.expander("Regimen-wide repeated PD concern domains"):
    ...
    if top_pairs:
        with st.expander("Pairwise concern highlights"):
