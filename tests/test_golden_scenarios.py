...
def test_pk_cyp3a4_strong_inhib_quetiapine_clarithro():
    ...
    assert any(h.rule_id == "PK_CYP3A4_STRONG_INHIB" and h.inputs["A"] == "quetiapine" for h in hits)


def test_pk_cyp3a4_strong_induc_midazolam_rifampin():
    _, hits = _run(["midazolam", "rifampin"])
    assert any(h.rule_id == "PK_CYP3A4_STRONG_INDUC" and h.inputs["A"] == "midazolam" for h in hits)


def test_pk_prodrug_activation_clopidogrel_fluconazole():
    _, hits = _run(["clopidogrel", "fluconazole"])
    assert any(h.rule_id == "PK_CYP2C19_INHIB_CLOPIDOGREL" and h.inputs["A"] == "clopidogrel" for h in hits)


def test_pk_pgp_digoxin_clarithro():
    _, hits = _run(["digoxin", "clarithromycin"])
    assert any(h.rule_id == "PK_PGP_INHIB_DIGOXIN" and h.inputs["A"] == "digoxin" for h in hits)
